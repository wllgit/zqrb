<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:84:"E:\demo\zqrb-server-php\public/../application/admin\view\advermanage\adverdraft.html";i:1535952502;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
                                ...
                                <div class="col-lg-10 self_selct">
                                    ...
                                    <div class="self_topic_selct">
                                        <select  class="form-control slt" id="self_selct_id" onchange="window.location=this.value"  name="column_order">
                                            <option <?php if($column[0]==0): ?>selected<?php endif; ?> value="<?php echo url('admin/Advermanage/adverDraft'); ?>?name=0">全部</option>
                                            <option <?php if($column[0]==1): ?>selected<?php endif; ?> value="<?php echo url('admin/Advermanage/adverDraft'); ?>?name=1">启动页广告</option>
                                            <option <?php if($column[0]==2): ?>selected<?php endif; ?> value="<?php echo url('admin/Advermanage/adverDraft'); ?>?name=2">banner页广告</option>
                                            <option <?php if($column[0]==3): ?>selected<?php endif; ?> value="<?php echo url('admin/Advermanage/adverDraft'); ?>?name=3">新闻列表广告</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    //删除下线广告
    function del_draft(id){
        layer.confirm("确认要删除吗，删除后不能恢复", { title: "删除确认" }, function (index) {
            layer.close(index);
            var jz;
            var url = "__URL__/advermanage/delDraft";
            $.ajax({
                type:"POST",
                url:url,
                data:{'id' : id},
                ...
                success: function(data) {
                    ...
                    if(data.code == 1){
                        ...
                        setTimeout("javascript:location.href='./adverDraft'", 1000);
                    }else{
    ...
    function getContent(){
        var jz;
        var url = "__URL__/advermanage/advertisementPublish";
        $.ajax({
            type:"POST",
            url:url,
